...
```bash
rkimgman unpack <input_file> <output_directory>
```

**Unpack RKFW firmware:**
```bash
$ rkimgman unpack rk.img ./out
...
```bash
$ rkimgman unpack ./out/embedded-update.img ./out
Filesize: 272773124
manufacturer:  RK3562
model:  RK3562
...
**Pack RKFW firmware:**
```bash
rkimgman pack-rkfw <input_directory> <output_file> [--chip <chip>] [--version <version>] [--timestamp <unix_timestamp>] [--code <code_field_hex>]
```
...
```bash
$ rkimgman pack-rkfw ./out ./repacked.img
```

Example (building from scratch):
```bash
$ rkimgman pack-rkfw ./out ./repacked.img --chip RK3562 --version 1.0.0 --timestamp 1762435994 --code 0x02000000
...
```bash
rkimgman pack-rkaf <input_directory> <output_file> --model <model> --manufacturer <manufacturer>
```
...
Example:
```bash
$ rkimgman pack-rkaf ./out repacked-update.img --model RK3562 --manufacturer RK3562
